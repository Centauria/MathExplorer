...
4. Your dispatch task text carries `agent_name` and `model` (your deterministic name embeds the model, e.g. `solver-k3-number-theory-abc-0af0`); the orchestrator stamps them into `results/<problem_id>/run.json` (`agent` field) after spawning — you do not record them yourself.
