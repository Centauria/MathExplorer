...
- 否则 problem_id 取第一个参数。若该题状态为 postponed：这是**恢复续攻**（resume）——分派前先读 `results/<problem_id>/run.json`，solver 需从上次迭代/phase 续攻（revival 语义）。
- 分派前先 `uv run python -m mathx.harvest set-status <problem_id> tackling`。
