...
2. 每次 worker 交付结果后：
   ...
   - CHAIN_ON 存在且无 hunter 在跑 → 按 .omp/commands/hunt.md 的流程派 hunter 补货（**持续补货，无水位上限**）；hunter 结算（ingest + mark-hunted，field 轮转）后若闸开且无 hunter 在跑 → 立即续派下一轮，直到 data/STOP / 链闸关 / quota gate 暂停（[quota] threshold）/ 用户叫停。hunter 每轮 yield 后结算再续派，不常驻。
