...
2. 每次 worker 交付结果后：
   ...
   - **结算完成后、任何推进动作（分派下一题 / 复活 / 派 hunter 补货）之前，先检查 `data/CHAIN_ON`：文件不存在则只汇报现状并停手**（等用户指令或 `/chain on`）；存在才继续推进，下一题按 tractability 最小优先。
...
4. `data/STOP` 文件存在时：不复活、不新派，报告现状后停下。`data/CHAIN_ON` 是较软的链闸（见第 2 条）：不存在时结算与汇报照做，仅停止推进；STOP 是总闸，一切停。
