...
16. Demand coverage: identify the problem's demanded claim/object at the start (what exactly must be proved, determined, or constructed) and keep it front and center throughout. The main theorem must establish exactly that claim — the referee panel enforces this (VERIFIER.md Phase 2.5) and rejects proofs whose conclusion merely restates the problem as open (e.g. "the exact value remains an open problem" when the Statement asks for the exact value). Never invoke `$verify-proof` and never mark a problem solved with a blueprint that leaves the demanded claim unsettled: the panel would be wasted and the verdict would be `wrong`. Keep working toward the demand instead, preserving partial progress; at the iteration cap, stop as `postponed` — do not claim success on partial results no matter how correct they are.
