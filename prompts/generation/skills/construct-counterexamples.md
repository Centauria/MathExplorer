...
- `uv run python -m mathx.memory append <problem_id> counterexamples @record.json`
...
- your own `web_search` / `web_fetch` and `uv run python -m mathx.leansearch "query"` to find standard counterexample patterns
