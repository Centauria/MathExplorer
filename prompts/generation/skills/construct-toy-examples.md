...
- `uv run python -m mathx.memory append <problem_id> toy_examples @record.json`
- `uv run python -m mathx.memory search <problem_id> "query" [--channels csv] [--limit 10]`
- `uv run python -m mathx.leansearch "query" [--num 10]` for matching examples/known motifs
- your own `web_search` / `web_fetch` for known example families and standard constructions
