...
1. Start with `uv run python -m mathx.leansearch "query" [--num 10]` (the ported `search_arxiv_theorems`).
2. Phrase the leansearch query as a complete mathematical statement whenever possible.
3. Inspect the returned items and decide whether they are useful for the current need.
4. If a useful theorem/example/counterexample is found and it comes from a paper, download that paper into the workspace (under `downloads/`, or fetch its arXiv page with `web_fetch`), extract its text, and read the extracted text before relying on the result.
...
8. Keep all downloaded papers and extracted text files inside `downloads/` in the repository.
9. Record not only what the theorem says, but also what its proof suggests for the current problem.
10. If leansearch exits with code 2 or returns no useful information, switch to your own `web_search` / `web_fetch`.
...
- your own `web_search` / `web_fetch` as the fallback retrieval path
